...
<div class="max-w-2xl mx-auto bg-white p-8 shadow">
    <h2 class="text-2xl font-bold mb-6">Criar Novo Grupo</h2>
        ...
        <div>
            ...
            <select id="departamento_id" name="departamento_id" class="w-full border rounded px-2 py-1 @error('departamento_id') border-red-500 @enderror" required>
                ...
                @foreach($departamentos as $dept)
                    <option value="{{ $dept->id }}" data-cnpj="{{ $dept->empresa?->cnpj_cpf ? preg_replace('/\D/', '', $dept->empresa->cnpj_cpf) : 'Não vinculado' }}" {{ old('departamento_id') == $dept->id ? 'selected' : '' }}>
