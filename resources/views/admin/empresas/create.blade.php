...
<div class="max-w-3xl mx-auto bg-white p-8 shadow">
    <h2 class="text-2xl font-bold mb-6">Criar Nova Empresa</h2>
    ...
    <form method="POST" action="{{ route('admin.empresas.store') }}" class="space-y-4">
        @csrf

        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label class="block font-semibold">CÓDIGO</label>
                <input type="text" value="Gerado automaticamente" class="w-full border rounded px-2 py-1 bg-gray-100 text-gray-600" disabled />
            </div>
            <div>
                <label class="block font-semibold">CNPJ/CPF</label>
                <input type="text" name="cnpj_cpf" value="{{ old('cnpj_cpf') }}" class="cnpj-cpf-mask w-full border rounded px-2 py-1 @error('cnpj_cpf') border-red-500 @enderror" maxlength="18" inputmode="numeric" required />
        ...
        <div>
            ...
            <input type="text" name="razaosocial" value="{{ old('razaosocial') }}" class="w-full border rounded px-2 py-1 @error('razaosocial') border-red-500 @enderror" required />
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="email" name="email" value="{{ old('email') }}" class="w-full border rounded px-2 py-1 @error('email') border-red-500 @enderror" required />
                @error('email')<p class="text-red-600 text-sm">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block font-semibold">FONE</label>
                <input type="text" name="fone" value="{{ old('fone') }}" class="fone-mask w-full border rounded px-2 py-1 @error('fone') border-red-500 @enderror" maxlength="15" inputmode="numeric" required />
            ...
            <div id="public-page-wrapper" class="rounded border border-sky-200 bg-sky-50 px-4 py-4 space-y-4 hidden">
                <div>
                    <label class="inline-flex items-center gap-2 text-sm font-semibold text-sky-900">
                        <input type="checkbox" name="public_page_enabled" value="1" class="rounded border-sky-300" @checked(old('public_page_enabled'))>
                ...
                <div>
                    ...
                    <input type="text" name="public_page_slug" value="{{ old('public_page_slug') }}" class="w-full border rounded px-2 py-1 @error('public_page_slug') border-red-500 @enderror" placeholder="minha-revenda">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="endereco" value="{{ old('endereco') }}" class="w-full border rounded px-2 py-1 @error('endereco') border-red-500 @enderror" />
                @error('endereco')<p class="text-red-600 text-sm">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block font-semibold">BAIRRO</label>
                <input type="text" name="bairro" value="{{ old('bairro') }}" class="w-full border rounded px-2 py-1 @error('bairro') border-red-500 @enderror" />
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="numero" value="{{ old('numero') }}" class="w-full border rounded px-2 py-1 @error('numero') border-red-500 @enderror" />
                @error('numero')<p class="text-red-600 text-sm">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block font-semibold">CEP</label>
                <input type="text" name="cep" value="{{ old('cep') }}" class="w-full border rounded px-2 py-1 @error('cep') border-red-500 @enderror" />
