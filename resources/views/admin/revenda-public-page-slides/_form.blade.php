...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Texto do botao</label>
                <input type="text" name="button_label" value="{{ old('button_label', $slide->button_label ?? '') }}" class="w-full border rounded px-3 py-2">
                @error('button_label')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Link do botao</label>
    ...
    <div class="rounded-xl border border-gray-200 bg-gray-50 p-4">
        <p class="text-sm font-semibold text-gray-800 mb-3">Previa</p>
        <div class="overflow-hidden rounded-xl bg-gray-900 min-h-[320px] relative">
            @if ($resolvedPreview)
                <img src="{{ $resolvedPreview }}" alt="Previa do slide" class="w-full h-80 object-cover">
            @else
            ...
            <div class="absolute left-0 right-0 bottom-0 p-6 text-white">
                <div class="text-2xl font-bold">{{ old('title', $slide->title ?? 'Titulo do slide') }}</div>
                <div class="mt-2 text-sm text-white/80">{{ old('subtitle', $slide->subtitle ?? 'Subtitulo opcional do slide.') }}</div>
            </div>
        </div>
    </div>
</div>
